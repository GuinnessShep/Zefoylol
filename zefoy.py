...
class Main:
    def __init__(self, proxy_list):
        self.proxy_iter = itertools.cycle(proxy_list)  # creates an iterator that cycles through the proxies
        self.options = Options()
        self.options.add_argument('-headless')  # Runs Firefox in headless mode
        self.proxy = Proxy()
        self.proxy.proxy_type = ProxyType.MANUAL
        self.proxy.http_proxy = next(self.proxy_iter)
        self.proxy.ssl_proxy = next(self.proxy_iter)
        capabilities = webdriver.DesiredCapabilities.FIREFOX
        self.proxy.add_to_capabilities(capabilities)
        self.driver = webdriver.Firefox(service=Service(GeckoDriverManager().install()), desired_capabilities=capabilities, options=self.options)
                self.xpaths = [
    ...
    def user_input_option(self):
        ...
        if s                return f"{Fore.GREEN}[ONLINE]{Fore.RESET}"
            else:
                return f"{Fore.RED}[OFFLINE]{Fore.RESET}"
        except TimeoutException:
            print("[-] 003 Error - Fatal error while generating list")
    ...
    def generate_and_send(self, text_box, search_button, send_button):
        ...
        try:
            while True:
                text_box = WebDriverWait(self.driver, 900).until(EC.text_to_be_present_in_element((By.XPATH, self.timer_text[self.option-1]), "Next Submit: READY....!"))
                time.sleep(3)
                search_button.click()
                send_button = WebDriverWait(self.driver, 5).until(EC.presence_of_element_located((By.XPATH, self.send_button[self.option-1])))
                time.sleep(5)
                ...
                self.successfully_message()
                text_box = None
                # refresh the WebDriver instance with a new proxy
                self.driver.quit()
                self.proxy.http_proxy = next(self.proxy_iter)
                self.proxy.ssl_proxy = next(self.proxy_iter)
                self.proxy.add_to_capabilities(capabilities)
...
def main():
    ...
    parser.add_argument('proxies', type=str, help='A file containing the list of proxies to use')
    args = parser.parse_args()

    proxy_list = load_proxies(args.proxies)

    main = Main(proxy_list)
    main.main()
